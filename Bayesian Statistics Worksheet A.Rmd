...
To explore these you should familiarise yourself with coding and running the lecture examples explained in the first week. These are reproduced here in R Markdown.
...
jags.mod.fit
```
Diagnostics: Look at the chain output for Y. Firstly extract the chain record and also map this into a time series object. Here histogram, chain time series record together with auto and partial correlation are explored. What do the acf terms help us to assess?
```{r}

y.diag <- jags.mod.fit$BUGSoutput$sims.list$Y
y.diag.ts <- ts(y.diag)

histogram(y.diag,xlab="Y",plot=TRUE)
ts.plot(y.diag.ts,xlab="Iteration",ylab="Y")
acf(y.diag.ts)
acf(y.diag.ts,type="partial")
...
print(jags.mod.fit)
```
Show the diagnostics (as above). Try altering the above code for differing chains and iteration lengths to see how the results and diagnostics alter. Discuss this with your Breakout Group colleagues.
```{r}
y.diag <- jags.mod.fit$BUGSoutput$sims.list$y
y.diag.ts <- ts(y.diag)
